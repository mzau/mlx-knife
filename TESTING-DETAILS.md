...
**Known limitation — single-process wet runs (nanobind "one process = one world"):** A full-tree wet run (`pytest -m wet` from the repo root) dies with a fatal nanobind abort (`refusing to add duplicate key "cpu" to enumeration "mlx.core.DeviceType"`) at the first in-process model load. Marker selection imports *every* test module at collection, so the unit-test stub machinery (`tests_2.0/stubs/`, plus the per-module stub/real swap `_use_real_mlx_modules`) shares one process with the real native MLX stack — and native extensions cannot be re-initialized. **`scripts/test-wet-umbrella.sh` avoids this by splitting Phase 1 path-scoped** (2026-06-17). The abort comes from the `live/` in-process tests (`test_streaming_parity` via `_use_real_mlx_modules`) initializing the real `mlx.core` in the same process as the collected stub tree, so —

- **Phase 1a** — `pytest -m wet tests_2.0/live -o addopts=""` (live dir = real world; the unit-stub tree is never collected)
- **Phase 1b** — `pytest -m wet tests_2.0 --ignore=tests_2.0/live --ignore=tests_2.0/test_stop_tokens_live.py -o addopts=""` (the non-live remainder)
- **Phase 1c** — `pytest -m live_stop_tokens tests_2.0/test_stop_tokens_live.py -o addopts=""` (stop-token regression, Issue #32 — also in-process real mlx, hence file-scoped; runs only under `-m live_stop_tokens`, with a per-model skip when a model/RAM is absent)

1b is safe even though it collects the stub tree: it `--ignore`s the one in-process-real-mlx file it would otherwise pull in (`test_stop_tokens_live`, owned by 1c), and those tests skip under `-m wet` anyway. Run wet by hand by mirroring 1a + 1b (+ 1c for the stop-token suite), not a bare `pytest -m wet`. The deeper structural fix (decide stub vs real at process start; delete `_use_real_mlx_modules`) is still planned.

> **Maintainer caveat — the path-split is an unguarded convention.** Any *new* test that initializes the real `mlx.core` in-process (a `live/` test using `_use_real_mlx_modules`, or a real model load) must run in its **own** path-scoped phase, never co-collected with the stub tree. There is no canary yet to enforce this, so an in-process-real-mlx test added into a stub-collecting selection **silently reintroduces the fatal abort** — surfacing as a confusing nanobind crash, not a clean assertion. Until the structural fix lands, treat "one path per in-process-real-mlx phase" as a hard rule when adding wet/live tests.
...
**Note:** These models are defined in `tests_2.0/live/test_utils.py` (`TEST_MODELS`, `VISION_TEST_MODELS`, `AUDIO_TEST_MODELS`, `EMBED_TEST_MODELS`) and `tests_2.0/test_stop_tokens_live.py` (`TEST_MODELS`). `EMBED_TEST_MODELS` (ADR-015, alpha-gated) holds the verified embedding representatives — one per code path / pooling branch — and is a test-fixture set, not a capability claim (verified classes live in `docs/MODEL-COVERAGE.md`).

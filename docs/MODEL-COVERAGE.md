...
| `pixtral`                                                                 | —    | ✅     | —     | ✅                   | 2.0.5       | Vision-chat tests in 2.0.5 wet-benchmark passed for three pre-quantized clones (`-4bit`, `-8bit`, `-bf16`); text-only prompts route cleanly to MLXRunner (ADR-020). **Convert-quantize verified mlxk-internal 2026-04-19:** `mlxk convert pixtral-12b-bf16 --quantize 6` → 10.8 GB target (6.803 bpw); `mlxk run` text + `mlxk run --image` both clean on the quantized output, no manual config repair needed. Note: text-only quality is not pixtral's strength — prefer a text-optimized model for text-only prompts. |
...
| `mllama`                                                                  | —    | ✅     | —     | 📚 (vlm)             | 2.0.5       | Vision-runtime **verified mlxk-internal 2026-04-19**: `Llama-3.2-11B-Vision-Instruct-4bit` with `mlxk run --image` produces coherent image descriptions (output quality below pixtral, but functional). **Caveat:** vision-only model — `mlxk run` *without* `--image` currently routes to mlx-lm (which correctly does not support mllama) and fails with misleading "model_type not supported"; `mlxk show` then reports `runtime incompatible` even though the vision path works. Known routing/health-aggregation bug (planned 2.0.6 fix). Use with `--image`. Convert-quantize not tested mlxk-internal. |
| `llava`, `llava_next`, `qwen2_vl`, `phi3_v`, `paligemma`, `idefics`, `smolvlm` | —    | 📚     | —     | 📚 (partial)         | 2.0.5       | In `VISION_MODEL_TYPES` (runtime). `llava`, `qwen2_vl` also in `VISION_QUANTIZE_TYPES`; rest are runtime-only. Not in this release's wet-benchmark. Likely affected by the same vision-only routing bug as `mllama` for text-only invocation; verify per type before relying on `mlxk show`. |
